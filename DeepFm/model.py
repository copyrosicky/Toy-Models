...
class DeepFM(nn.Module):
    def __init__(self, cate_fea_uniques,
                 ...
                 dropout=[0.2, 0.2]):
        '''
        :param cate_fea_uniques: 输入列表格式 列表元素为每个类别特征对应的的类别数
        :param num_fea_size: 连续特征的维度
        :param emb_size: FM部分隐向量的维度
        :param hidden_dims: Deep部分 每一层hidden layers的维度
        :param num_classes:  模型最终预测的类别数
        :param dropout:
        '''
        ...
        ])  # 直接使用embedding层 输出为sparse feature对应的类别 输出1维的hidden vector vocsize为feature的类别总数
    ...
    def forward(self, X_sparse, X_dense=None):
        ...
        fm_1st_sparse_res = torch.cat(fm_1st_sparse_res, dim=1)  # torch.Size([bs, cate_fea_size])
        ...
        # 使用经过化简的FM二阶特征向量计算公式 最终输出k维的向量 即省略了公式中的求和项
        # 先求和再平方
